  ...
  return (
    <Container>
      ...
      <div className="flex gap-4 text-gray-400 items-center mb-4">
        <Link to={`/heroes/${urlParametize(build.hero.name)}`}>
          {build.hero.name}
        </Link>
        <span>•</span>
        ...
        <div className="flex gap-2">
          <Form method="post">
            ...
            <Button
              ...
              className="bg-transparent text-indigo-500 inline-flex gap-1 items-center">
              {loggedInUserHasBuildLike ? (
                <svg
          ...
          {isLoggedInUsersBuild ? (
            <LinkButton
              to={`/builds/${build.hero.name}/${build.name}/edit`}
